...
colnames(drug) <- c("SB334867_score")
drug
```

```{r}
pca_loadings <- data.frame(combined@reductions$pca@feature.loadings)
write.csv(pca_loadings, paste0(home_dir, "/pca_loadings.csv"))
```
...
```{r}
write.csv(drug, file = paste0(home_dir, "/drug_orthologs/SB33_orthologs.csv"), row.names = TRUE)
```
...
```{r}
#write.csv(drug_mouse_cor_pca, file = paste0(home_dir, "/correlations/SB334867_pca_nofilter.csv"))
write.csv(drug_mouse_cor_pca, file = paste0(home_dir, "/correlations/SB334867_pca_filter.csv"))
```
